...
class BaseTestCase(BaseCase):
    ...
    def click_button(self, btn_loc, wait=False, loading=False):
        """
        单独点击一个按钮
    ...
    def click_and_jump(self, btn_loc, handle_num, wait=False, loading=False):
        """
        点击按钮并跳转新开tab
        :param btn_loc: 按钮定位器
    ...
    def __choice_point(self, subject=False):
        if subject:
            self.click_button(f'//div[@class="el-cascader-panel"]'
                              f'/div[1]/div[1]/ul/li[{PointIdIndex.checkpoint_level_one_index}]', wait=True)
            self.click_button(f'//div[@class="el-cascader-panel"]'
                              f'/div[2]/div[1]/ul/li[{PointIdIndex.checkpoint_level_two_index}]')
        else:
            self.click_button(f'//div[@class="el-cascader-panel"]'
                              f'/div[1]/div[1]/ul/li[{PointIdIndex.level_one_index}]', wait=True)
    ...
    def subject_add_course_simple(self, package_name, discover=False) -> str:
        ...
        self.__assert_add_course_tip('发布课件成功', ElementSelector.succ_tip_loc,
                                     ElementSelector.repeated_tip_confirm_loc)
        check_point_course_name = self.get_text(ElementSelector.check_point_course_name_loc) \
            if discover else self.get_text(ElementSelector.first_course_name_loc)
        if discover:
            self.driver.close()
    ...
    def subject_add_homework_simple(self, homework_name, answer_config, timing=None):
        ...
        self.add_text(ElementSelector.checkpoint_homework_name_input_loc, text=homework_name)
        ...
        if 1 == timing:
            ...
            self.add_text(ElementSelector.checkpoint_timing_input_loc,
                          self.__input_time(start=True))
            self.send_keys(ElementSelector.checkpoint_timing_input_loc,
                           Keys.ENTER)

        self.update_text(ElementSelector.checkpoint_end_time_input_loc,
                         self.__input_time())
        self.send_keys(ElementSelector.checkpoint_end_time_input_loc,
                       Keys.ENTER)
        self.click_button(ElementSelector.public_homework_btn_loc)
        self.__assert_equal('发布成功！', ElementSelector.succ_tip_loc)
        self.wait_for_text_visible(homework_name)
    ...
    def subject_student_check_course_simple(self, course_name, discover=False):
        ...
        for btn in self.course_btn_list:
            try:
                self.click_button(f'//p[text()="{btn}"]', loading=True)
                if discover:
                    self.click_button(
    ...
    def subject_student_do_homework_simple(self, homework_name):
        ...
        try:
            self.wait_for_element_visible(ElementSelector.go_to_code_btn_loc)
            ...
            self.click_and_jump(ElementSelector.go_to_code_btn_loc, 2, loading=True)
    ...
    def student_do_homework_loop(self):
        ...
        for a in range(1, 4):  # 依次做作业列表3个作业
            ...
            try:
                ...
                self.click_button(homework_name_sel)
            self.click(ElementSelector.view_code_btn_loc)
            ...
            self.wait_for_text_visible('评测有误', ElementSelector.unpass_result_text_loc)
    ...
    def subject_student_do_homework_loop(self):
        ...
        for a in range(1, 4):  # 依次做作业列表3个作业
            try:
                self.wait_for_element_visible(  # 作业去答题
            ...
            self.click_button(f'//div[@class="el-row"]/div[{n}]')
            code_input = self.get_element(ElementSelector.code_view_loc)
            wrong_answer = 'wrong_answer = "wrong"'
            code_input.send_keys(wrong_answer)
            ...
            if a == 3:
                ...
                for w in range(1, n + 1):
                    try:
                        self.wait_for_element_visible(ElementSelector.wrong_problem_name_loc)
                    except Exception as e:
                        print(f'{e}作业作答作业列表为空，刷新后重新点击')
                        self.refresh()
                    finally:
                        wrong_problem_name_num = self.get_text(ElementSelector.wrong_problem_name_loc)
                        ...
                        problem_name = self.get_text(ElementSelector.problem_name_loc)
            ...
            self.wait_for_text_visible('已完成', ElementSelector.checkpoint_homework_status_loc)
    ...
    def click_china_map(self):
        ...
        try:
            self.click_button(f'//div[@class="level"]'
    ...
    def click_map_path(self):
        ...
        :return:None
        """
        self.click_button(f'//div[@class="level"]/div[{PointIdIndex.checkpoint_level_two_index}]')
    ...
    def date_selection(self, page, name, student=False):
        ...
        try:
            self.wait_for_text_visible(name)
        except ElementNotVisibleException:
            print(f'没有搜索到{name}')
    ...
    def search_input(self, name):
        ...
        :return: None
        """
        self.add_text(ElementSelector.search_input_loc, text=name)
    ...
    def __do_homework_operation(self, num, problem_id_list, problem_type='', subject=False):
        ...
        if subject:
            for i in range(1, num + 1):
                try:
                    self.wait_for_element_visible(f'//div[@class="el-row"]/div[{i}]')
                except Exception as e:
                    print(f'{e}作业作答作业列表为空，刷新后重新点击')
                    self.refresh()
                finally:
    ...
    def __do_challenge_operation(self, subject=False):
        ...
        for i in range(5):
            if subject:
                if i == 0:
                    problem_name = self.get_text(ElementSelector.enm_problem_name_loc)
                else:
                    problem_name = self.get_text(ElementSelector.enm_problem_name_loc_1)
            else:  # 标准授课取出题目名称
                problem_name_list = self.get_text(ElementSelector.standard_enm_problem_name_loc)
            ...
            if i == 4:  # 主题授课从第5题开始点击 继续挑战 -> 换一题
                if subject:
                    for n in range(3):
                        ...
                        self.wait_for_text_visible(problem_name)
                        ...
                        try:
                            self.wait_for_text_visible('挑战成功', ElementSelector.challenge_result_tip_loc)
                        except ElementNotVisibleException:
                            print(f'题目"{problem_name}"答案错了，用挑战失败再断言一次')
                            self.wait_for_text_visible('挑战失败', ElementSelector.challenge_result_tip_loc)
                        except Exception as e:
                            print(f'{e}挑战结果异常')
            else:  # 点击保存并评测按钮之后的操作
    ...
    def __subject_push_homework_operation(self):
        ...
        for v in evaluation_list:
            try:
                self.wait_for_text_visible(v, ElementSelector.result_tip_loc)
                print(f'作业评价是{v}')
            except ElementNotVisibleException:
                print(f'作业评价不是 {v}')
            except Exception as e:
    ...
    def __check_course_operation(self, btn):
        if '课件' == btn:
            try:
                frame_elem = self.get_element(ElementSelector.iframe_loc)
    ...
    def audit_work(self, work_name):
        ...
        self.click_button(f'//div[text()="{work_name}"]', loading=True)
        if '直接审核作品' == work_name:
    ...
    def __pic_image_identify_operation(self):
        ...
        for t in btn_text_list:
            if self.__wait_for_loading():
                self.click(f'//span[contains(text(),"{t}")]')
                try:
                    if '人脸' == t:
                        if self.__wait_for_loading():
    ...
    def add_account_class(self, username_teacher, teacher_name, username_student, student_name, admin_class_name,
        ...
        self.assert_equal('成功添加！', ElementSelector.add_success_tip_loc)
    ...
    def add_resources(self):
        ...
        for visibility in options_list:
            ...
            self.click_button(f'//span[text()="{visibility}"]')
            ...
            self.wait_for_text_visible('添加成功')
    ...
    def add_draft(self):
        ...
        for n in range(0, 10):
            ...
            if 0 == n:
                self.add_text(ElementSelector.draft_name_input_loc, text=n)
            else:
                self.update_text(ElementSelector.draft_name_input_loc, new_value=n)
    ...
    def do_test_field(self, model):
        ...
        :return: None
        """
        code_input_element = self.get_element(ElementSelector.ace_text_input_loc)
        ...
        if 'pygame' == model:
            try:
                ...
                try:
                    out_text = self.get_text(ElementSelector.text_out_area_loc)
            ...
            try:
                self.wait_for_text_visible('abc', ElementSelector.text_out_area_loc)
            except Exception as e:
                print(e)
        self.update_text(ElementSelector.draft_name_input_loc, new_value=f'{model}测试')
        self.click_button(ElementSelector.save_btn_loc)
    ...
    def multiple_files_test_field(self, file_name, draft_name, output):
        ...
        self.add_text(ElementSelector.create_file_input_loc, text=file_name)
        ...
        try:
            ...
            print(f'{e}运行失败，输出错误')
        self.update_text(ElementSelector.draft_name_input_loc, new_value=draft_name)
        self.click_button(ElementSelector.save_btn_loc)
        self.click_button(ElementSelector.save_confirm_btn_loc)
    ...
    def open_file(self, output):
        ...
        draft_name = self.get_text(ElementSelector.first_draft_loc)
    ...
    def three_dimensional(self):
        ...
        """
        self.switch_to_window(1)
        self.hover_on_element(ElementSelector.type_choose_loc)
        ...
        self.click_button(ElementSelector.run_code_btn_loc)
        try:
            self.wait_for_element_visible(ElementSelector.ck_type_output_loc)
        except Exception as e:
            print(f'{e}3D建模异常，没有输出')
        self.update_text(ElementSelector.draft_name_input_loc, new_value='3D建模测试')
        self.click_button(ElementSelector.save_btn_loc)
        self.click_button(ElementSelector.save_confirm_btn_loc)
    ...
    def robot(self):
        ...
        self.click_button(ElementSelector.run_code_btn_loc)
        try:
            self.wait_for_element_visible(ElementSelector.robot_img_loc)
        except Exception as e:
            print(f'{e}机器人图像异常')
    ...
    def check_error(self):
        ...
        input_code(code, code_input_element)
        self.click_button(ElementSelector.run_code_btn_loc)
        self.wait_for_text_visible('Error', ElementSelector.text_out_area_loc)
    ...
    def submit_work(self, work_name):
        ...
        :return: None
        """
        name_input_elem = self.get_element(ElementSelector.work_name_input_loc)
    ...
    def edit_material_name(self, material_name):
        ...
        self.add_text(ElementSelector.material_name_input_loc, text=material_name)
        self.click_button(ElementSelector.upload_confirm_btn_loc)
        try:
            self.wait_for_text_visible(material_name, ElementSelector.material_name_loc)
        except Exception as e:
            print(f'{e}编辑素材名称异常')
